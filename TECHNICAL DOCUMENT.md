# VTEX ↔ Shopify ↔ 3PL Order & Tracking Integration
...
- **Shopify Admin API version `2024-01` or later**, fulfillment orders model. Earlier API versions and apps installed before January 2026 may follow a different (legacy) integration contract — see §2.4. Any future Shopify API version bump should be re-validated against this flow, particularly `move.json` and `fulfillment_request.json`'s accepted payload.
...
1. **Create (or reuse) the custom app** (app available on my Git) in the Shopify Partner Dashboard. A single custom app definition can be reused across multiple store installs — each install generates its own access token, so there is no need to create a new app per store.
